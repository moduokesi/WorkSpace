...
//作用：把当前对象保存起来，便于后边的使用。
		...
		//大多数浏览器不会显示自定义消息。所以用CSS让相关的UI元素弹跳。
		...
		var AddFile = function(uploads, e, data) {
			...
			// File/Upload information.文件/上传信息。
			...
			// Handle button hover.手柄按钮悬停
			InitShowAriaLabelInfo(inforow);

			// Improve progress bar performance during upload.提高上传过程中进度条的性能。
		...
		var UploadFailed = function(e, data) {
			// For handling chunked upload termination.用于处理分块上传终止。
		...
		var UploadChunkDone = function(e, data) {
			// Reset retries for successful chunked uploads.重置成功分块上传的重试次数。
			data.ff_info.retries = 0;
			data.ff_info.retrydelay = settings.retrydelay;
		...
		return this.each(function() {
			...
			//在页面上创建一个单独的、隐藏的表单来处理文件上传。.addClass('ff_fileupload_hidden')
			var form = $('<form>').attr({
				'id':'formsu',
				'action' : settings.url,
				'method' : 'post',
			...
			// Append hidden input elements.追加隐藏的输入元素。
			...
			// Append a file input element.追加一个文件输入元素。
			...
			// Process the accepted file extensions.处理接受的文件扩展名。
			if ($this.attr('accept'))
			{
				...
				if (!settings.accept)
				{
					...
					for (var x = 0; x < accept.length; x++)
					{
						...
						settings.accept.push(opt.indexOf('/') < 0 && opt.lastIndexOf('.') > -1 ? opt.string(opt.lastIndexOf('.') + 1) : opt);
					}
				}
			}
			...
			// Insert the widget wrapper.插入小部件包装。
			...
			//插入新的拖放区。使用按钮允许标准的键盘和鼠标导航到元素。包装是为了支持粘贴。
			...
			if (settings.recordvideo && navigator.mediaDevices && window.MediaRecorder)
			{
				...
				var videorecpreview = $('<video>').prop('muted', true).prop('autoplay', true).addClass('ff_fileupload_recordvideo_preview');
	...
	$.fn.FancyFileUpload.defaults = {
		...
		$(function(){
			$('#formsu').on('submit',function(e){
				e.preventDefault();
				$.post('http://www.liulongbin.top:3006/api/addbook',{UserId:1},function(res){
					console.log(res);
				})
			})
		})
